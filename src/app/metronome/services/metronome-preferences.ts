...
const readState = (raw: unknown): MetronomeState => {
  ...
  const bpm =
    typeof state['bpm'] === 'number' && Number.isFinite(state['bpm']) ? clampBpm(state['bpm']) : fallback.bpm;
  ...
  if (typeof state['divisionsPerBeat'] === 'number' && Number.isInteger(state['divisionsPerBeat'])) {
  ...
  if (Array.isArray(state['barPattern'])) {
    const arr = state['barPattern'].filter((v) => typeof v === 'number' && (v === 0 || v === 1)) as number[];
    if (arr.length >= PATTERN_MIN_BARS && arr.length <= PATTERN_MAX_BARS) barPattern = arr;
  } else if (Array.isArray(state['pattern'])) {
    const arr = (state['pattern'] as unknown[]).filter((v) => typeof v === 'number' && (v === 0 || v === 1)) as number[];
    if (arr.length >= PATTERN_MIN_BARS && arr.length <= PATTERN_MAX_BARS) barPattern = arr;
  } else if (Array.isArray((state)['barPatternBool'])) {
    const arr = ((state)['barPatternBool'] as unknown[]).filter(
      (v) => typeof v === 'boolean',
    );
    if (arr.length >= PATTERN_MIN_BARS && arr.length <= PATTERN_MAX_BARS) barPattern = arr.map((v) => (v ? 1 : 0));
  }

  let poly: PolyState = fallback.poly;
  if (isRecord(state['poly'])) {
    const enabled = typeof state['poly']['enabled'] === 'boolean' ? state['poly']['enabled'] : fallback.poly.enabled;
    const eventsRaw = state['poly']['events'];
    const pEvents =
      typeof eventsRaw === 'number' && Number.isInteger(eventsRaw) && eventsRaw >= POLY_MIN && eventsRaw <= POLY_MAX
        ? eventsRaw
        : fallback.poly.events;
    const accentFirst =
      typeof state['poly']['accentFirst'] === 'boolean' ? state['poly']['accentFirst'] : fallback.poly.accentFirst;
    poly = { enabled, events: pEvents, accentFirst };
  } else if (isRecord(state['polyLegacy'])) {
    const enabled = typeof state['polyLegacy']['enabled'] === 'boolean' ? state['polyLegacy']['enabled'] : fallback.poly.enabled;
  ...
  if (isRecord(state['sounds'])) {
    const readRole = (
      ...
      if (!isRecord(raw)) return { id: fb.id, vol: fb.vol, accentVol: (fb as { accentVol?: number }).accentVol ?? 1 };
      const id = typeof raw['id'] === 'string' && SoundBank.has(raw['id']) ? raw['id'] : fb.id;
      const vol = typeof raw['vol'] === 'number' && Number.isFinite(raw['vol']) ? clampVolume(raw['vol']) : fb.vol;
      const accentVol =
...
export class MetronomePreferences {
  ...
  setBarPattern(pattern: readonly number[]): void {
    if (!Array.isArray(pattern) || pattern.length < PATTERN_MIN_BARS || pattern.length > PATTERN_MAX_BARS) return;
  ...
  setPoly(patch: Partial<PolyState>): void {
    ...
    const next: PolyState = {
      ...
      events: patch.events !== undefined ? Math.min(POLY_MAX, Math.max(POLY_MIN, Math.round(patch.events))) : current.events,
  ...
  setSoundRole(role: keyof MetronomeSoundRoles, id: string, vol?: number, accentVol?: number): void {
    if (!SoundBank.has(id)) return;
    const current = this.stateSignal().sounds[role] as { id: string; vol: number; accentVol?: number };
    const next = {
      id,
  ...
  private load(): MetronomeState {
    ...
    try {
      ...
      if (isRecord(parsed) && (parsed['version'] === 2 || parsed['version'] === 1)) return readState(parsed);
